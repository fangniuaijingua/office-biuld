...
本仓库发布通用技能逻辑、检查脚本和参考规则。除下方经用户明确批准、仅用于展示版式效果的预览图及新增通用示意图外，不包含用户模板、可编辑报告/PPT 成品、原始照片、日志或项目数据。
...
| 载体 | 默认风格 | 不变的约束 |
| --- | --- | --- |
| 实验报告 | 正文/表格按模板；技术图黑白、灰阶或少量低饱和棕色；保留少量必要彩照 | 不自动套蓝色标题/表头，不重画原始证据 |
| 技术 PPT | 构思时比较裁切、比例和材质组合；锁定后保持一致 | 不随机修改事实、语义色、已认可页面 |
| 地图示意 | 地图与说明栏分离，禁区与裕量统一坐标、裁剪至绘图区 | 近似示意明确标注，不为美观缩小真实禁区 |
| 网上选图 | 按主题、构图、色温及用途定向检索，登记来源和使用状态 | 权属不明仅参考，不冒充项目实拍 |

![技术图低彩度与地图绘制边界的通用示意](assets/showcase/figure-style-guide.png)

上图为**程序生成的通用规则示意**，没有使用项目照片、精确场地或实验数据，不是实际成品或性能证据。保留[可复现图源](assets/showcase/render_figure_style_guide.py)。原有报告/PPT效果图继续保留，不将示意图包装为新成品。

已认可设计方向后，只做裁切、对齐、字重、色温和必要材料层次的局部微调。使用前仍需逐页渲染；规则更新不等于保证每个项目首次生成都通过审美验收。
