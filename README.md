...
本仓库仅发布通用技能逻辑、检查脚本和参考规则；不包含任何用户模板、实验报告、PPT 成品、照片、日志或项目数据。
...
5. 用户需要图片效果展示时，仅从已通过质检的最终 PDF 派生 `showcase/vN/` 逐页 PNG 与联系表；不改写报告正文，也不把预览图提交到公共仓库。
...
提交到此公开仓库前，务必确认不含用户文档、照片、日志、账号信息、绝对本机路径或生成的交付物。
