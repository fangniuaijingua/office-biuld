...
Finish 是可重复的版式语法，不是任意贴纸。每页通常选择一至两类；必要时增加协调的图像材质，以层级和可读性约束，不设机械数量上限：
...
小型 Finish 标记通常占页面视觉面积约 10% 以内；低对比的图片/文档背景按视觉重量和可读性判断，不机械套用面积上限。使用既有强调 token 或中性色。每个元素都要在 Page Lock 写下它服务的“定位、框定、节奏、深度”之一；说不出作用就删除。若 Hero 已经复杂，则删去纹理，只保留来源轨和一条框定线。
...
- 随机性只用于构思：在既有网格、字体、语义色和证据边界内，比较 2—3 种构图/裁切/材质组合并选择最贴合内容的一种。不要随机改变技术关系、数据颜色、页码位置或已确认页；选定后写入 Page Lock，后续重渲染保持一致。使用随机采样脚本时记录 seed。
- 检查相邻页与全套联系表：变化来自主图比例、阅读方向、图片出血、局部特写、透明材料层和明暗节奏，而非只换 RGB。连续对比页允许一致；不设强制每页换色或同色最多出现几次的机械指标。
- 除图注和细线外，允许大面积但低对比的相关图片裁切、文档扫描、柔和纸面材质、照片叠层、局部放大与细节带。按页面需要选用，不将类别库变成每页必加清单；图像承担层次，文字和证据仍最清楚。
- 备料阶段若现有素材不足以形成完整画面，主动网上检索相关、协调、可使用的图片。检索包含主体、画幅、光线/色温、负空间方向和背景/前景用途；先查看候选原图，再决定裁切。不要只搜“高级 PPT”，也不要下载有水印的模板缩略图填空。
- 将选用图片保存为本地资产，记录原始页面 URL、作者/机构、许可或使用状态、用途与裁切。权属不明的图只作构图参考，不嵌入交付；网络不可用时列出具体缺料，不假装已找到素材。外部/生成图不能冒充项目实拍。
- 有效留白应保留；大片空白若仅因缺素材则先调整主图尺度与支持材料。装饰不足和装饰抢正文同样需要内部修正；透明度调整后检查投影尺度可见性、文字对比、边缘与图层遮挡。


## 已确认方向后的审美微调

用户认可整套方向后，停止重新探索全套风格；只调整图片裁切/明度、间距、对齐、字重、局部色温和必要的材料层次。以联系表检查全套和谐，再检查单页；装饰丰富不应造成新母题不断增加，既有页和结论不因随机变化被重建。用户指定问题页时优先局部修订，不自动重做全套。
