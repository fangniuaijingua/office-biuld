...
- 分离物理坐标、绘图区和说明区；矩形统一为 x/y/width/height，不把右上端点误作宽高。
- 先确认地图版本、单位、纵横比和禁区来源；未知尺寸只画标明“近似/示意”的图，不猜测成精确地图。不同代码版本与报告参数不能悄悄混用。
- 禁区与膨胀边界在同一坐标系生成；膨胀区域与场地取交集并 clip 到绘图区，说明栏使用独立布局轴。不能靠缩小真实禁区来修复视觉遮挡。
- 检查矩形边界、路线与禁区相交、目标标记、图例越界；导出后检查原图和插入 Word 后的实际页面。仅修图时优先替换对应媒体部件，核对正文、样式、其他图片未改变。
